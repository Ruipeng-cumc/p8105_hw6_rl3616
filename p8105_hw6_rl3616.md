...
```
    ...
    ## 
    ## Call:
    ## lm(formula = bw_formula_main, data = birthweight_df)
    ## 
    ## Residuals:
...
```
    ...
    ## 
    ## Call:
    ## lm(formula = bw_formula_small, data = birthweight_df)
    ## 
    ## Residuals:
    ##     Min      1Q  Median      3Q     Max 
...
```
    ...
    ## 
    ## Call:
    ## lm(formula = bw_formula_big, data = birthweight_df)
    ## 
    ## Residuals:
    ##      Min       1Q   Median       3Q      Max 
...
To evaluate predictive performance, we used Monte Carlo cross-validation
with 10 random train–test splits generated by `crossv_mc.` For each
split, the models were fit on the training set and tested on the
held-out data using `purrr::map2_dbl` to compute RMSE.
...
```
    ...
    ## # A tibble: 1 × 3
    ##   main_rmse small_rmse big_rmse
    ##       <dbl>      <dbl>    <dbl>
    ## 1      285.       341.     294.

As result, our proposed main model achieved the lowest average RMSE
(≈285 g), indicating the best out-of-sample predictive performance among
the three models.

The small model had substantially higher prediction error (≈341 g),
while the interaction model performed slightly worse than the main model
(≈294 g).
